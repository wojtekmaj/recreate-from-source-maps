...
const extractFilesFromMaps = async (sourceMapsContent) => {
  ...
  const files = unmergedFiles.reduce((obj, newFiles) => {
    const existingFiles = Object.keys(newFiles).filter(
      (newFile) => obj[newFile] && obj[newFile] !== newFiles[newFile],
    );
    if (existingFiles.length) {
      warning(
        `The following files will be overwritten with different content: ${existingFiles.join(
          ', ',
        )}.`,
      );
    }
